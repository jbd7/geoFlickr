<?php
	...
	add_action('wp_print_scripts','myscriptvars');

	function myscriptvars() {
	?>
	<script type="text/javascript">
		var api_key = "<?php echo get_option('flickrapikey') ?>";
	...
	function load_scripts() {
			...
			$flickrToolbarJs =  plugins_url() . '/geoflickr/js/flickr_toolbar.js';

			wp_register_script('flickrToolbarJs', $flickrToolbarJs);
			wp_enqueue_script( 'flickrToolbarJs');
	...
	function load_styles() {
			...
			$flickrToolbarCss =  plugins_url() . '/geoflickr/css/flickr_toolbar.css';

			wp_register_style('flickrToolbarCss', $flickrToolbarCss);
			wp_enqueue_style( 'flickrToolbarCss');
	...
	function geoflickr_init(){
		register_setting( 'geoflickr_plugin_options', 'flickrapikey');
	}
	...
	<div class="wrap">
		...
		<h2>geoFlickr by <a href="https://github.com/jbd7/geoFlickr/" target="_blank">jbd7</a></h2>
			<form method="post" action="options.php">
				...
				<?php $options = get_option('flickrapikey'); ?>
				<table class="form-table">
					<tr>
						<th scope="row">Flickr API Key</th>						
						<td>
							<input type="text" size="57" name="flickrapikey" value="<?php echo $options; ?>" />
						</td>
					</tr>
					<tr>
						<th scope="row"></th>
	...
	function geoflickr_plugin_action_links( $links, $file ) {
		...
		if ( $file == plugin_basename( __FILE__ ) ) {
			$posk_links = '<a href="'.get_admin_url().'options-general.php?page=geoflickr/geoflickr.php">'.__('Settings').'</a>';
